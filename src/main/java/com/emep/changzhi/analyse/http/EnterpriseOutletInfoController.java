 ...
 */
@RestController
@RequestMapping("enterpriseoutletinfo")
@Api(value = "aqi controller",tags = "AQI控制器")
public class EnterpriseOutletInfoController {
    ...
    public ResultBean<Collection<EnterpriseOutletInfo>> queryUser(){
        return new ResultBean<Collection<EnterpriseOutletInfo>>(enterpriseOutletInfoService.list());
    }
    ...
    public ResultBean<SysAqiInfo> queryUser(@RequestParam Map<String, Object> payload){
        ...
        if (result.getCode() == Constant.RESULT_CODE_SUCCESS) {
            return  result.ok(enterpriseOutletInfoService.findEnterpriseOutletInfoById(payload.get("id").toString()));
        }
        return  result;
    }
    ...
    public ResultBean<Boolean> delete(@RequestParam Map<String, Object> payload){
        ResultBean result = DataValidator.validateMapData(payload, "id");
        if (result.getCode() == Constant.RESULT_CODE_SUCCESS){
            return result.bool(enterpriseOutletInfoService.delete(payload.get("id").toString()));
        }
        return result;
    }
    ...
    public ResultBean<Page<SysAqiInfo>> showUsers(@RequestParam Map<String, Object> payload) {
        ...
        if (result.getCode() == Constant.RESULT_CODE_SUCCESS){
            Page<EnterpriseOutletInfo> pageEnterpriseOutletInfo = enterpriseOutletInfoService.findAll(Integer.parseInt(payload.get("pageIndex")
                    .toString()),Integer.parseInt(payload.get("pageSize").toString()));
            return result.ok(pageEnterpriseOutletInfo);
        }
        return result;
    }
    ...
    @GetMapping("findEnterpriseOutletInfoQuery")
    public ResultBean<Page<EnterpriseOutletInfo>> PageUsers(@RequestParam Map<String, Object> payload) {
        ResultBean result = DataValidator.validateMapData(payload, "pageIndex","pageSize","aqiName");
        if (result.getCode() == Constant.RESULT_CODE_SUCCESS){
            EnterpriseOutletInfo enterpriseOutletInfo = new EnterpriseOutletInfo();
            enterpriseOutletInfo.setOutletName(payload.containsKey("aqiName") ?payload.get("aqiName") != null ? payload.get("aqiName").toString():"":"" );
            Page<EnterpriseOutletInfo> pageSysAqiInfo = enterpriseOutletInfoService.findBookCriteria(Integer.parseInt(payload.get("pageIndex") .toString()),Integer.parseInt(payload.get("pageSize").toString()),enterpriseOutletInfo);
    ...
    public ResultBean<Integer> updateAA(@RequestParam Map<String, Object> payload) {
        ...
        if (result.getCode() == Constant.RESULT_CODE_SUCCESS){
            Integer aa = enterpriseOutletInfoService.updateOutletName(payload.get("id") .toString(),payload.get("aqi") .toString());
            return result.ok(aa);
        }
        return result;
    }
